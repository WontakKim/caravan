...
|-------|------|---------------|
| `caravan-core` | `crates/caravan-core` | TUI-free logic: commands, events, storage, prompt compiler, runner, model layer |
| `caravan-tui` | `crates/caravan-tui` | App state, input handling, and rendering (depends on `caravan-core`) |
| `caravan-bin` | `crates/caravan-bin` | Binary entrypoint; produces the `caravan` binary (depends on `caravan-core` and `caravan-tui`) |
...
Run/Turn event assembly to `crates/caravan-core/src/runner.rs`.
...
The `ModelAdapter` trait (defined in `crates/caravan-core/src/model.rs`) exposes a single method:
...
`runner.rs`/`crates/caravan-core/src/model.rs`/gateway wiring change rather than an App-layer change.

## ModelGateway Boundary

`ModelRequest is now defined in` `crates/caravan-core/src/model.rs` as the shared core adapter request type used by `ModelAdapter`, `ModelAdapterRegistry`, `ModelGateway`, and the runner — no longer a gateway-local type.
...
integrations require changes only inside `ModelGateway` and `crates/caravan-core/src/model.rs`,
...
plain strings. Both are defined in `crates/caravan-core/src/model_types.rs`:
...
`OpenAICompatibleAdapter` lives in `crates/caravan-core/src/model_openai_compatible.rs` and
...
Two new typed variants have been added to the enums in `crates/caravan-core/src/model_types.rs`:
...
`crates/caravan-core/src/model_openai_types.rs` defines the five serde-serializable structs that
...
`OpenAICompatibleConfig` in `crates/caravan-core/src/model_openai_config.rs` is the configuration
...
`OpenAIRequestBuilder::build` in `crates/caravan-core/src/model_openai_request.rs` combines an `OpenAICompatibleConfig`,
