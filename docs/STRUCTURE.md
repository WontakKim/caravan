...
| **Read-only workspace tools** | `crates/kernel/src/tool/registry.rs`, `crates/tui/src/app/tools.rs` | `/tool list` enumerates registered tools; `/tool read <path>` reads a file from the workspace and supports reading a line range (offset and limit) in addition to whole-file reads; `/tool search <query>` searches for text across workspace files (`search_text`); `/tool glob <pattern>` finds files matching a glob pattern (`glob_files`). All four are read-only and auto-allowed without an approval step. These are the only tool commands that belong to the baseline surface. |
| **Workspace Reference resolver** | `crates/kernel/src/workspace_reference.rs` | Parses `@path` tokens (e.g. `@README.md`, `@crates/kernel/src/`) out of plain-text input and resolves each one, read-only, by calling `ToolRegistry::execute` directly — reusing the same path-safety confinement (absolute-path and `..` rejection, symlink-escape detection) as `/tool read`/`/tool list`. A file reference optionally supports an inclusive line range suffix (`:N`, `:N-M`, `#LN`, `#LN-LM`), reusing the same `read_file` offset/limit path as `/tool read` and capped at the `MAX_READ_RANGE_LIMIT_LINES` (500) line-count limit. This is baseline input UX, **not a slash command**: it never reaches `ToolEventRunner` and emits **no** `ToolPolicy`/`ToolCall`/`ToolResult`/`ToolError`/`ToolContextAttach` event. Resolved references render a `Referenced Workspace Context` block in the compiled prompt — a sub-block distinct from the `/tool`-driven `Attached Workspace Context` block rendered from `ManualToolContext`. Resolution is scoped to the current turn only; it is not staged as pending context for a later message. |
