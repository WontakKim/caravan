...
| **Read-only workspace tools** | `crates/kernel/src/tool/registry.rs`, `crates/tui/src/app/tools.rs` | `/tool list` enumerates registered tools; `/tool read <path>` reads a file from the workspace; `/tool search <query>` searches for text across workspace files (`search_text`); `/tool glob <pattern>` finds files matching a glob pattern (`glob_files`). All four are read-only and auto-allowed without an approval step. These are the only tool commands that belong to the baseline surface. |
