...
| **Workspace Reference resolver** | `crates/kernel/src/workspace_reference.rs` | Parses `@path` tokens (e.g. `@README.md`, `@crates/kernel/src/`) out of plain-text input and resolves each one, read-only, by calling `ToolRegistry::execute` directly — reusing the same path-safety confinement (absolute-path and `..` rejection, symlink-escape detection) as `/tool read`/`/tool list`. This is baseline input UX, **not a slash command**: it never reaches `ToolEventRunner` and emits **no** `ToolPolicy`/`ToolCall`/`ToolResult`/`ToolError`/`ToolContextAttach` event. Resolved references render a `Referenced Workspace Context` block in the compiled prompt — a sub-block distinct from the `/tool`-driven `Attached Workspace Context` block rendered from `ManualToolContext`. Resolution is scoped to the current turn only; it is not staged as pending context for a later message. |
